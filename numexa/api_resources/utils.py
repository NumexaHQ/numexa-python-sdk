...
class NumexaApiPaths(str, Enum, metaclass=MetaEnum):
...
def default_api_key() -> str:
    if numexa.api_key:
        return numexa.api_key
    env_api_key = os.environ.get(NUMEXA_API_KEY, "")
    if env_api_key:
        return env_api_key
...
def default_base_url() -> str:
    if numexa.base_url:
        return numexa.base_url

    env_base_url = os.environ.get(NUMEXA_PROXY_URL, NUMEXA_DIRECT_URL)
    if env_base_url:
        return env_base_url
